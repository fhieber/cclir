...
if [[ ! -f $INIT_WEIGHTS || ! -f $SOURCE_QUERIES || ! -f $TREC_RELS || ! -f $DOCUMENT_VECTORS || ! -f $DFTABLE || ! -f $CDEC_INI ]]; then
	echo "one or more files given in the .ini file are missing!"
	exit 1
fi
if [[ -d $WORKDIR ]]; then
...
# binary folder
BINDIR=/toolbox/cdec_fh/training/pltrain
...
echo "Epoch $E - start $(date)"
	...
	$BINDIR/retrieval -q $E.instances -c documents.tf -j 22 -d documents.df -o $E.clir -k $TOPK -n $N -a $AVG_LEN -r $E >> out.log 2>> err.log
	...
	echo " optimizing ..."
	$BINDIR/sgd -i $E.instances.labels -w weights.$PREV_E -o weights.$E -t $MAX_ITER -e $EPS -b $BATCH_SIZE -l $LEARNING_RATE > losses.$E 2>> err.log 
