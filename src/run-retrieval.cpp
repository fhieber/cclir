 ...
 * run-retrieval.cpp
 *
 *  runs a set of query on the document vectors coming from STDIN.
 *  Queries are loaded in memory.
 *  run-retrieval can be used as a mapper script for MapReduce
 */


#include "run-retrieval.h"
...
int main(int argc, char** argv) {
	...
	// load queries
	vector<Document> queries;
	loadQueries(cfg["queries"].as<string>(), queries);
	cerr << queries.size() << " queries loaded." << endl;

	// load topic map
	/*boost::unordered_map<string, string> topic_map;
	if (cfg.count("topic-map"))
		loadQueryIDPACtMap(cfg["topic-map"].as<string>(), topic_map);
	*/
	...
	vector<Scores<string> > scores (queries.size(), Scores<string>(cfg["K"].as<int>()));
...
/*
 * loads the topic map file. Format is
 * PACt\tUCID\n
 */
