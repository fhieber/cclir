...
def main():
  ...
  parser.add_argument('--rel_weights', required=True, help='relevance weights')
  ...
  shutil.copy(args.rel_weights,os.path.join(args.output_dir,'weights.relevance'))
...
def optimize(args, script_dir, dev_size):
  ...
  decoder = script_dir+'/hgbest_cut_mira_relevance'
  ...
  while i<args.max_iterations:
    ...
    if args.verbose:
      decoder_cmd += ' -v'
    decoder_cmd += ' --rel_weights %s'%(args.output_dir+'/weights.relevance')
