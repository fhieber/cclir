...
def scaleRelevanceWeightsToSMTWeights(relevanceweightfile, weightfile, scalingfactor):
	s = 0.0
...
def optimize(args, script_dir, source, source_size, references):
	...
	parallelize = script_dir+'/../utils/parallelize.pl'
	decoder = script_dir+'/online_hg_mira'
	best_score_iter = -1
	best_score = -1
	i = 0
	...
	#if args.scaling != 0: # scale relevance weights to length of smt weights
	...
	while i<args.max_iterations:
		...
		curr_pass = '%02d'%i
		decoder_cmd = ('{0} -c {1} -w {2} -r {3} -n {4} -o {5} -h {6} -f {7} --optimizer {8} --scaling {9} --scalingfactor {10}').format(
				   decoder, args.config, weightsfile, relevanceweightsfile,
				   args.learningrate, weightdir, args.hope, args.fear, args.optimizer, args.scaling, args.scalingfactor)
		if args.freeze != []:
		...
		#new_weights_file_avg = '%s/weights.%02d.avg'%(args.output_dir, i+1)
		last_weights_file = '%s/weights.%02d'%(args.output_dir, i)
		i += 1
		weight_files = glob.glob(weightdir +'/last.*')
		#weight_files_avg = glob.glob(weightdir +'/avg.*')
		average_weights.average_weights(new_weights_file, weight_files, False)
		#average_weights.average_weights(new_weights_file_avg, weight_files_avg, True)
...
def main():
	...
	parser.add_argument('--hope', type=int, default=1, choices=range(1,3), help='how to select hope candidate. options: 1) ramp=1, 2) ssvm=2')
	parser.add_argument('--fear', type=int, default=1, choices=range(1,4), help='how to select fear candidate. options: 1) model score + cost, 2) max cost, 3) max score')
	parser.add_argument('--optimizer', type=int, default=3, choices=range(1,5), help='choose optimizer: Optimizer (SGD=1, PA MIRA w/Delta=2, Cutting Plane MIRA=3, PA MIRA=4)')
	parser.add_argument('--max-iterations', type=int, default=20, metavar='N', help='maximum number of iterations to run')
	parser.add_argument('-l','--learningrate', type=float, default=0.001, help='learning rate / step size')	
	...
	parser.add_argument('-s', '--scaling', type=int, default=1, choices=range(0,3), help='scale rw to ||w||: 0 (disabled), 1 (rescale at initializiation), 2 (rescale after each update of w).')	
	parser.add_argument('--scalingfactor', type=float, default=1, help='x in rw = (rw / ||w||) * x')
