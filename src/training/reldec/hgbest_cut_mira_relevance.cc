...
bool InitCommandLine(int argc, char** argv, po::variables_map* conf) {
  ...
  opts.add_options()
    ...
    ("rel_weights", po::value<string>(), "Input relevance weights")
...
inline double vscale(double v, double vmin, double vmax) {
	if (vmin == vmax) return 1;
	return (v-vmin) / (vmax-vmin);
}

struct TrainingObserver : public DecoderObserver {
  TrainingObserver(const int k, SparseVector<weight_t>* w, SparseVector<weight_t>* rw, vector<GoodBadOracle>* o ) :  weights(w), rel_weights(rw), oracles(*o), kbest_size(k) {}

  SparseVector<weight_t>* weights;
  SparseVector<weight_t>* rel_weights;
 ...
 const HypothesisInfo& GetCurrentOracle() const {
  ...
  virtual void NotifyTranslationForest(const SentenceMetadata& smeta, Hypergraph* hg) {
    ...
    if(unique_kbest)
      UpdateOracles<KBest::FilterUnique>(cur_sent, *hg);
    else
      UpdateOracles<KBest::NoFilter<std::vector<WordID> > >(cur_sent, *hg);
    forest = *hg;
  ...
  void UpdateOracles(int sent_id, Hypergraph& forest) {

    if (stream) sent_id = 0;
    bool PRINT_LIST= false;
    cerr << "UpdateOracle sent_it: " << sent_id << endl;
    cerr << "UpdateOracle oracles.size()=" << oracles.size() << endl;
    ...
    double max_rel = std::numeric_limits<double>::lowest();
	double min_rel = std::numeric_limits<double>::max();

    typedef KBest::KBestDerivations<vector<WordID>, ESentenceTraversal,Filter> K;
    // t_score kbest (hg weighted with current weights)
    ...
    for (int i = 0; i < kbest_size; ++i) {
      ...
      // relevance score
      double rel = mt_metric_scale * d->feature_values.dot(*rel_weights);
      if (rel>max_rel) max_rel = rel;
      if (rel<min_rel) min_rel = rel;
      
      cur_best.push_back( MakeHypothesisInfo(d->feature_values, rel, d->yield));
      best_size++;
    }
    ...
    // get hope kbest list
    if(hope_select == 1) {
    	forest.Reweight(*weights + *rel_weights);
    } else {
    	forest.Reweight(*rel_weights);
    }
    K kbest2(forest,kbest_size);
    unsigned hope_size = 0;
    for (int i = 0; i < kbest_size; ++i) {
    	typename K::Derivation *d =
    			kbest.LazyKthBest(forest.nodes_.size() - 1, i);
		if (!d) break;
		...
		if (rel<min_rel) min_rel = rel;
		hope_size++;
		cur_good.push_back(MakeHypothesisInfo(d->feature_values, rel, d->yield));
    }
    cerr << "min_rel=" << min_rel << " max_rel=" << max_rel << endl;
    ...
    if(fear_select == 1) {
    	...
    	SparseVector<weight_t> tmprw = *rel_weights;
    	tmprw *= -1;
    	forest.Reweight(tmprw);
    }
    K kbest3(forest,kbest_size);
    unsigned fear_size = 0;
    for (int i = 0; i < kbest_size; ++i) {
		typename K::Derivation *d =
				kbest.LazyKthBest(forest.nodes_.size() - 1, i);
		if (!d) break;
		...
		if (rel<min_rel) min_rel = rel;
		fear_size++;
		cur_bad.push_back(MakeHypothesisInfo(d->feature_values, rel, d->yield));
	}
    cerr << "min_rel=" << min_rel << " max_rel=" << max_rel << endl;

    cerr << best_size + fear_size + hope_size << " hypotheses in 3 lists!\n";

    // scale relevance between 0 and 1
    for(unsigned u=0;u!=cur_good.size();++u)
    	cur_good[u]->mt_metric = vscale(cur_good[u]->mt_metric, min_rel, max_rel);
    for(unsigned u=0;u!=cur_bad.size();++u)
    	cur_bad[u]->mt_metric = vscale(cur_bad[u]->mt_metric, min_rel, max_rel);
    for(unsigned u=0;u!=cur_best.size();++u)
    	cur_best[u]->mt_metric = vscale(cur_best[u]->mt_metric, min_rel, max_rel);
    
    boost::shared_ptr<HypothesisInfo>& oracleN = cur_good[0];
    double hope_score = oracleN->features.dot(dense_w_local);
    // compute hope and fear values
    for(unsigned u=0;u!=cur_good.size();++u) {
    	cur_good[u]->hope = cur_good[u]->mt_metric + cur_good[u]->features.dot(dense_w_local);
    	double t_score = cur_good[u]->features.dot(dense_w_local);
    	cur_good[u]->fear = -1*cur_good[u]->mt_metric + 1*oracleN->mt_metric - hope_score + t_score; // relative loss
    	cur_good[u]->oracle_loss = -1*cur_good[u]->mt_metric + 1*oracleN->mt_metric;
    	cur_good[u]->oracle_feat_diff = oracleN->features - cur_good[u]->features;
    	cur_good[u]->oracleN=oracleN;
    	for (SparseVector<weight_t>::iterator it=rel_weights->begin(); it!=rel_weights->end(); ++it) cur_good[u]->features.set_value(it->first, .0);
    }
    for(unsigned u=0;u!=cur_bad.size();++u) {
        cur_bad[u]->hope = cur_bad[u]->mt_metric + cur_bad[u]->features.dot(dense_w_local);
    	double t_score = cur_bad[u]->features.dot(dense_w_local);
    	cur_bad[u]->fear = -1*cur_bad[u]->mt_metric + 1*oracleN->mt_metric - hope_score + t_score; // relative loss
    	cur_bad[u]->oracle_loss = -1*cur_bad[u]->mt_metric + 1*oracleN->mt_metric;
    	cur_bad[u]->oracle_feat_diff = oracleN->features - cur_bad[u]->features;
    	cur_bad[u]->oracleN=oracleN;
    	for (SparseVector<weight_t>::iterator it=rel_weights->begin(); it!=rel_weights->end(); ++it) cur_bad[u]->features.set_value(it->first, .0);
    }
    for(unsigned u=0;u!=cur_best.size();++u) {
        cur_best[u]->hope = cur_best[u]->mt_metric + cur_best[u]->features.dot(dense_w_local);
    	double t_score = cur_best[u]->features.dot(dense_w_local);
    	cur_best[u]->fear = -1*cur_best[u]->mt_metric + 1*oracleN->mt_metric - hope_score + t_score; // relative loss
    	cur_best[u]->oracle_loss = -1*cur_best[u]->mt_metric + 1*oracleN->mt_metric;
    	cur_best[u]->oracle_feat_diff = oracleN->features - cur_best[u]->features;
    	cur_best[u]->oracleN=oracleN;
    	for (SparseVector<weight_t>::iterator it=rel_weights->begin(); it!=rel_weights->end(); ++it) cur_best[u]->features.set_value(it->first, .0);
    }
...
void loadRelevanceWeights(const string& fname, SparseVector<weight_t>& rw) {
	rw.clear();
...
int main(int argc, char** argv) {
  ...
  SparseVector<weight_t> rw;
  loadRelevanceWeights(conf["rel_weights"].as<string>(), rw);
  cerr << "|RW|=" << rw.size() << endl;
  ...
  cerr << "oracles size: " << oracles.size() << endl;

  BasicObserver bobs;
  TrainingObserver observer(conf["k_best_size"].as<int>(), &lambdas, &rw, &oracles);
  ...
  if (VERBOSE) cerr << lambdas << endl; 
  ScoreP acc, acc_h, acc_f;
  
  while(*in >> id) {
	  in->ignore(1, '\t');
      getline(*in, buf);
      if (buf.empty() || id.empty()) continue;
		...
		if(optimize_again)
		  {
			...
			for (SparseVector<weight_t>::iterator it=rw.begin(); it!=rw.end(); ++it)
					lambdas.erase(it->first);
