...
def main():
  ...
  parser.add_argument('-i', '--input', required=True, help='input trianing set')
  ...
  parser.add_argument('--qrels', required=True, type=str, help='qrels for testing')
  parser.add_argument('--servers', required=True, type=str, nargs='+', default={}, help='list of retrieval servers')
...
def optimize(args, script_dir, dev_size):
  ...
  decoder = script_dir+'/kbest_cut_mira_ir'
  source = args.output_dir+'/input'
  with open(args.output_dir+'/refs', 'w') as fref:
    for i in xrange(dev_size):
      fref.write("bla\n")
    fref.close()
  refs = args.output_dir+'/refs'
  ...
  while i<args.max_iterations:
    ...
    if args.verbose:
      decoder_cmd += ' -v'
    decoder_cmd += ' --qrels %s'%(args.qrels)
    decoder_cmd += ' --servers %s'%(" ".join(args.servers))
