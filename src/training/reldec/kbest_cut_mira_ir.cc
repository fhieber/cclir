...
bool InitCommandLine(int argc, char** argv, po::variables_map* conf) {
  ...
  opts.add_options()
    ...
    ("servers", po::value<vector<string> >()->multitoken(), "list of server connections, e.g. localhost:5555, donna:5558,...")
...
struct TrainingObserver : public DecoderObserver {
  TrainingObserver(const int k,
		  	  	  	  vector<GoodBadOracle>* o, CLIR::IRScorer* irscorer, zmq::socket_t* socket ) :
		  	  	  		  oracles(*o), kbest_size(k), irs(irscorer), client(socket) {
	  runid = "1";
	  resultsize = 1000;
 ...
 const HypothesisInfo& GetCurrentOracle() const {
  ...
  double ComputeIRScore(const vector<WordID>& yield) {
	  string translation = TD::GetString(yield);
	  irs->reset();
	  CLIR::Document d(translation, qid);
	  assert(d.parsed_ == true);
	  d.computeTfVector(false);
	  string reply;
	  DISTCLIR::run_query(DISTCLIR::construct_msg(runid, resultsize, false, d.asVector()), client, reply);
	  irs->evaluateSegment(reply);
	  double ndcg = irs->NDCG();
	  irs->reset();
	  //cerr << "NDCG SCORE=" << ndcg << endl;
	  return ndcg;
  }
  ...
  void UpdateOracles(int sent_id, const Hypergraph& forest) {
    ...
    cerr << "UpdateOracle sent_id: " << sent_id << endl;
    cerr << "UpdateOracle qid: " << qid << endl;
    ...
    for (int i = 0; i < kbest_size; ++i) {
      ...
      // get IR score
      float sentscore = mt_metric_scale * ComputeIRScore(d->yield);
...
int main(int argc, char** argv) {
  ...
  vector<string> servers = conf["servers"].as<vector<string> >();
  zmq::context_t context (1);
  zmq::socket_t* client = DISTCLIR::setup_socket(context, servers);
  CLIR::IRScorer irs(conf["qrels"].as<string>());

  TrainingObserver observer(conf["k_best_size"].as<int>(), &oracles, &irs, client);
  ...
  while(*in >> id) {
      ...
      cerr << "SENT: " << cur_sent << " QID:" << observer.qid << endl;
