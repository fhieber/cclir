...
#include "instance.h"
#include "../../vector-projection/util.h"
...
bool init_params(int argc, char** argv, po::variables_map* cfg) {
	po::options_description cl("\nApplies Gold Labels to Instances...");
	cl.add_options()
			("queries,q", po::value<string>(), "file containing Instances as queries.")
...
void loadLabels(const string& dirname, boost::unordered_map<string,vector<double> >& labels, const string& label_suffix=".labels") {
	...
	while ((dirp = readdir(dp)) != NULL) {
		...
		if (fname != "."
			&& fname != ".."
			&& vutils::endsWith(fname, label_suffix) ) {
			std::stringstream ss;
