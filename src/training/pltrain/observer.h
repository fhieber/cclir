...
struct InstanceGetter : public DecoderObserver {
	...
	Instance& GetTrainingInstance() { return instance_; }
...
struct KbestInstanceGetter : public InstanceGetter {
	...
	void KbestGet(const Hypergraph& forest, Instance& instance) {
		...
		for (int i=0; i<k_;++i) {
			...
			if (/*i == k_-1 ||*/ cur_chunksize == chunksize_) { // start new chunk
				...
				CLIR::Query query(nbest_ttable);
				query.constrain(L_, C_, swf_);
				instance.AddDerivation(query.asString(), feature_vector, chunk_score, weights_);
				// reset
				cur_chunksize = 0;
...
struct ForestSampleInstanceGetter : public InstanceGetter {
	...
	void KSample(const Hypergraph& forest, Instance& instance) {
		...
		for (unsigned i = 0; i < k_; ++i) { // for each sample (TODO implement chunksize)
			...
			CLIR::Query query(nbest_ttable);
			query.constrain(L_, C_, swf_);
			instance.AddDerivation(query.asString(), samples[i].fmap, samples[i].model_score.v_, weights_);
		}
	}
