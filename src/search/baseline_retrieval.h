 ...
 * DOCID\tDOCLEN\tVECTOR\n
 */
void loadQueries(const string& fname, vector<CLIR::Document>& queries) {
	ReadFile f(fname);
	string docid;
	int len;
	string raw;
	while(*f >> docid) {
		...
		*f >> len;
		...
		queries.push_back( CLIR::Document(vutils::read_vector(raw), docid, len) );
	}
}
...
prob_t const bm25(const TermVector& Q, const TermVector& D, const int Dlen, CLIR::BM25* scorer, const DfTable& dft) {
	prob_t s = prob_t(0);
