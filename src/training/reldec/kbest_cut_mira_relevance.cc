...
struct TrainingObserver : public DecoderObserver {
  TrainingObserver(const int k, const SparseVector<weight_t>& rw, vector<GoodBadOracle>* o ) :  rel_weights(rw), oracles(*o), kbest_size(k) {}
 ...
 const HypothesisInfo& GetCurrentOracle() const {
  ...
  void UpdateOracles(int sent_id, const Hypergraph& forest) {
	...
	if(PRINT_LIST)cerr << TD::GetString(d->yield) << " ||| " << d->score << " ||| " << rel << endl;
	cur_best.push_back( MakeHypothesisInfo(d->feature_values, rel, d->yield));
      }
      
      all_hyp.push_back(MakeHypothesisInfo(d->feature_values, rel, d->yield));   //store all hyp to extract hope and fear
    }

    cerr << "min_rel=" << min_rel << " max_rel=" << max_rel << endl;
    // scale relevance between 0 and 1 and remove relevance features from vectors
    for(unsigned u=0;u!=all_hyp.size();++u) {
    	all_hyp[u]->mt_metric = vscale(all_hyp[u]->mt_metric, min_rel, max_rel);
    	for (SparseVector<weight_t>::iterator it=rel_weights.begin(); it!=rel_weights.end(); ++it) all_hyp[u]->features.set_value(it->first, .0);
    }
    ...
    //sort all hyps by sentscore (eg. relevance)
...
int main(int argc, char** argv) {
  ...
  BasicObserver bobs;
  TrainingObserver observer(conf["k_best_size"].as<int>(), rw, &oracles);
  ...
  while(*in >> id) {
	  ...
	  if (loss > 0.0 || !checkloss) {
	    ...
	    for (SparseVector<weight_t>::iterator it=rw.begin(); it!=rw.end(); ++it) lambdas.erase(it->first);
